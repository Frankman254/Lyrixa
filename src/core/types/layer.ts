...
/**
 * Spellings other producers use for a role Lyrixa already has a name for.
 *
 * `romanization` is the word the transcription side of the pipeline uses for
 * romaji/pinyin output; it means exactly `transliteration` here. Accepting the
 * alias on input costs nothing and keeps a hand-written or third-party file
 * from silently losing its role — but only the canonical name is ever written
 * back out, so LiveWallpaper never has to learn two words for one thing.
 */
const LYRIC_LAYER_ROLE_ALIASES: Readonly<Record<string, LyricLayerRole>> = {
...
/**
 * Coerce an untrusted value into a role, resolving known aliases.
 * Returns `undefined` for anything unrecognized: an invented role would
 * travel into the lyrics bundle and make a renderer branch on garbage.
 */
export function coerceLyricLayerRole(value: unknown): LyricLayerRole | undefined {
  if (isLyricLayerRole(value)) return value;
  if (typeof value !== 'string') return undefined;
  return LYRIC_LAYER_ROLE_ALIASES[value.trim().toLowerCase()];
}

/** Human label for a role, used by the layer list and inspector. */
export const LYRIC_LAYER_ROLE_LABELS: Readonly<Record<LyricLayerRole, string>> = {
  primary: 'Primary',
...
/**
 * Roles whose text is derived from a primary line and can therefore be
 * regenerated by a translation service. Used to decide which clips get
 * marked stale when their primary line is edited.
 */
export const DERIVED_TEXT_ROLES: readonly LyricLayerRole[] = [
  'translation', 'transliteration'
];

export function isDerivedTextRole(role: LyricLayerRole | undefined): boolean {
  return role !== undefined && (DERIVED_TEXT_ROLES as readonly string[]).includes(role);
}

/**
 * BCP-47-ish tidy-up. We deliberately do not validate against a registry:
 * the producer knows its own language codes, and rejecting an unfamiliar one
 * would lose information the renderer might still display.
 */
...
/**
 * A display name for a layer that leads with what it *is*, not what it was
 * named. `Translation · es` reads correctly even when the author left the
 * layer called "Backing Vocals".
 */
export function describeLyricLayer(layer: Pick<LyricLayer, 'name' | 'role' | 'language'>): string {
  const parts: string[] = [];
  if (layer.role) parts.push(LYRIC_LAYER_ROLE_LABELS[layer.role]);
  if (layer.language) parts.push(layer.language);
  return parts.length ? parts.join(' \u00b7 ') : layer.name;
}
